...
$(() =>
{
    ...
    function populateDiscussionWithData(discussion)
    {
        ...
        {
            userIcon.remove();
            $('#user').prepend($('<img>', { src: `${window.location.origin}:3001/uploads/${discussion.picturePath}` }));
        }
    ...
    function createCommentHeader(userName, id, userPicture, date, edited)
    {
        ...
        if (userPicture)
            commentHeader.append($('<img>', { src: `${window.location.origin}:3001/uploads/${userPicture}` }));
        else
